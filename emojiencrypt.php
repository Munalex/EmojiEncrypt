<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['entrada'])) {
        ...
        for ($i = 0; $i < strlen($caracteres); $i++) {
            $cadena = $caracteres[$i];
            if (array_key_exists($cadena, $emojis)) {
                $resultado .= $emojis[$cadena];
            }
        }
    }
?>
...
<body>
    <div>
        <form action="emojiencrypt.php" method="post">
            <textarea name="entrada" id="entrada" cols="71" rows="15"
                placeholder="Escribe aquí lo que quieras encriptar con emojis"></textarea>
